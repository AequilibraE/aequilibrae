...
void print_usage(const char *program) {
  std::cerr << "Usage: " << program
            << " [num_elements > 0] [--csv] [--csv-header-only]" << '\n';
}
...
void print_csv_header() {
  std::cout << "elements,decrease_key_ops,std_ms,adapter_ms,fourary_ms,pairing_"
               "ms,adapter_vs_std,fourary_vs_std,pairing_vs_std,status"
            << '\n';
}

std::vector<Update> build_updates(std::vector<double> &keys,
...
RunResult run_std_reference(const std::vector<double> &initial_keys,
...
RunResult run_queue(const std::vector<double> &initial_keys,
                    const std::vector<Update> &updates,
                    std::vector<std::size_t> &order,
                    std::vector<double> &keys) {
  RunResult result;
  ...
  for (std::size_t i = 0; i < initial_keys.size(); ++i) {
    ...
    if (queue.effective_state(idx) != ElementState::SCANNED) {
      result.ok = false;
      result.message =
          "state mismatch (expected SCANNED) at step " + std::to_string(i);
      return result;
    }
...
int main(int argc, char **argv) {
  ...
  const RunResult std_result =
      run_std_reference(initial_keys, updates, expected_order, expected_keys);
  if (!std_result.ok) {
  ...
  const RunResult four_ary_result =
      run_queue<FourAryHeap>(initial_keys, updates, order, keys);
  if (!four_ary_result.ok) {
    std::cerr << "FourAryHeap failed: " << four_ary_result.message << '\n';
    return 1;
  }
  const RunResult four_ary_validation =
      validate_against_reference(order, keys, expected_order, expected_keys);
  if (!four_ary_validation.ok) {
    std::cerr << "FourAryHeap validation failed: "
              << four_ary_validation.message << '\n';
    return 1;
  }

  const RunResult pairing_result =
      run_queue<PairingHeap>(initial_keys, updates, order, keys);
  if (!pairing_result.ok) {
    std::cerr << "PairingHeap failed: " << pairing_result.message << '\n';
    return 1;
  }
  const RunResult pairing_validation =
      validate_against_reference(order, keys, expected_order, expected_keys);
  ...
  const RunResult adapter_result =
      run_queue<StdPriorityQueueAdapter>(initial_keys, updates, order, keys);
  if (!adapter_result.ok) {
    std::cerr << "StdPriorityQueueAdapter failed: " << adapter_result.message
              << '\n';
    return 1;
  }
  const RunResult adapter_validation =
      validate_against_reference(order, keys, expected_order, expected_keys);
  if (!adapter_validation.ok) {
    std::cerr << "StdPriorityQueueAdapter validation failed: "
              << adapter_validation.message << '\n';
    return 1;
  }

  const double adapter_vs_std =
      std_result.ms > 0.0 ? adapter_result.ms / std_result.ms : 0.0;
  const double four_ary_vs_std =
      std_result.ms > 0.0 ? four_ary_result.ms / std_result.ms : 0.0;
  const double pairing_vs_std =
      std_result.ms > 0.0 ? pairing_result.ms / std_result.ms : 0.0;

  std::cout << std::fixed << std::setprecision(3);
  if (options.csv) {
    std::cout << n << ',' << updates.size() << ',' << std_result.ms << ','
              << adapter_result.ms << ',' << four_ary_result.ms << ','
              << pairing_result.ms << ',' << adapter_vs_std << ','
              << four_ary_vs_std << ',' << pairing_vs_std << ',' << "ok"
              << '\n';
  } else {
