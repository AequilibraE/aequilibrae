...
"""Reusable all-or-nothing assignment with optional skimming."""
...
cdef class AoNOutputs:
    """Store link loads, skims and turn costs for one assignment run.
    ...
    def __init__(self, links, zones, classes, fields):
    ...
    def copy(self):
        ...
        return self.copy_to(AoNOutputs(*self.shape))
...
cdef class PreparedAoN:
    ...
    and run() for each iteration. Do not change inputs or reuse objects during a run.
    """
    ...
    def run(self, AoNOutputs out not None):
        ...
        if out.shape != self.shape:
            raise ValueError("output shape does not match assignment shape")
