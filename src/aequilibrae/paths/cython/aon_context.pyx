...
cdef class PreparedAoN:
    ...
    def __init__(
        ...
        *,
        ...
        self.heap_type = routing_heap_from_name(heap)
...
cdef void assign_origin(
    ...
    run_dijkstra(routing, query.search, worker.search, heap_type)
...
cdef void assign_origins(
    ...
    for index in prange(<Py_ssize_t>inputs.origin_count, num_threads=cores, schedule="guided"):
        assign_origin(routing, inputs.origins[index], inputs, workers[threadid()], output, heap_type)
