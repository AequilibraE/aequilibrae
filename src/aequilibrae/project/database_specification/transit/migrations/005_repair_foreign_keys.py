...
def migrate(
    *,
    ...
    if transit_conn is None:
        raise RuntimeError("Transit migration 005 requires a transit_conn connection")

    existing_tables = {row[0] for row in transit_conn.execute("SELECT name FROM sqlite_master WHERE type='table'")}
