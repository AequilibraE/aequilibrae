...
class MultiThreadedAoN:
    ...
    def prepare(self, graph, results):
        ...
        self.temporary_skims = np.zeros(
            (results.nodes, results.num_skims, results.cores), dtype=ftype
        )
        ...
        self.temp_link_loads = np.zeros(
            (results.links, results.classes["number"], results.cores), dtype=ftype
        )
        self.temp_node_loads = np.zeros(
            (results.nodes, results.classes["number"], results.cores), dtype=ftype
        )
        self.temp_b_nodes = np.zeros(
            (graph.b_node.shape[0], results.cores), dtype=itype
        )
