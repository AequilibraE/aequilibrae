  ...
  for i in prange(l, nogil=True, num_threads=cores):
      if link_flows[i] > 0:
        if link_flows[i] > capacity[i]:
          congested_time[i] = fftime[i] * (1 + alpha[i] * (pow(link_flows[i] / capacity[i], 2*beta[i])))
        else:
...
cpdef void dbpr2_cython(double[:] deltaresult,
  ...
  for i in prange(l, nogil=True, num_threads=cores):
    if link_flows[i] > 0:
      if link_flows[i] > capacity[i]:
        deltaresult[i] = fftime[i] * (alpha[i] * 2 * beta[i] * (pow(link_flows[i] / capacity[i], (2*beta[i])-1)))/ capacity[i]
      else:
