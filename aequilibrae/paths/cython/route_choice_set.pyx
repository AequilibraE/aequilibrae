...
cdef class RouteChoiceSet:
    ...
    def __init__(self, graph: Graph):
        ...
        # copy=True materialises a fresh, writable, numpy-backed buffer so Cython memoryview
        # binding succeeds regardless of the pandas backend (CoW, PyArrow-backed columns, etc.)
        self.b_nodes_view = graph.compact_graph.b_node.to_numpy(copy=True)
        ...
        self.ids_graph_view = graph.compact_graph.id.to_numpy(copy=True)

        # We explicitly don't want the links that have been removed from the graph
        self.graph_compressed_id_view = graph.graph.__compressed_id__.to_numpy(copy=True)
