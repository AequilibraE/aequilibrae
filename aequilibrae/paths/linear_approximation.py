...
class LinearApproximation(WorkerThread):
    ...
    def execute(self):  # noqa: C901
        ...
        self.logger.info("Iteration, RelativeGap (AON), RelativeGap (BFW direction), stepsize")
        ...
        self.logger.info(
            ...
            f"final AON rgap = {self.rgap}, final BFW-direction rgap = {self.rgap_direction}"
        )
    ...
    def __objective_change_at_stepsize(self, stepsize: float) -> float:
        ...
        link_term = 0.5 * stepsize * sum_a_times_b_minus_c(
            self._trap_avg_cost,
            self.step_direction_flow,
            self.fw_total_flow,
            self.cores,
