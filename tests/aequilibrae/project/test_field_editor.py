...
def table(empty_project, table_name):
    return FieldEditor(empty_project.scenario.db_connection, table_name)
...
def test_save(empty_project, table_name, attribute):
    table = FieldEditor(empty_project.scenario.db_connection, table_name)
    random_val = "some_value"
    setattr(table, attribute, random_val)
    table.save()
    table2 = FieldEditor(empty_project.scenario.db_connection, table_name)
