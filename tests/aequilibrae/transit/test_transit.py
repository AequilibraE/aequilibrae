...
def test_new_gtfs_builder(build_gtfs_project):
    ...
    with build_gtfs_project.transit_connection as conn:
        existing = conn.execute("SELECT COALESCE(MAX(DISTINCT(agency_id)), 0) FROM agencies;").fetchone()[0]

    transit = build_gtfs_project.transit.new_gtfs_builder(
        agency="Agency_1",
        day="2016-04-13",
    ...
    transit2 = build_gtfs_project.transit.new_gtfs_builder(
        agency="Agency_2",
        day="2016-07-19",
        file_path=join(build_gtfs_project.project_base_path, "gtfs_coquimbo.zip"),
    )
    ...
    transit3 = build_gtfs_project.transit.new_gtfs_builder(
        agency="Agency_3",
        day="2016-07-19",
        file_path=join(build_gtfs_project.project_base_path, "gtfs_coquimbo.zip"),
    )
